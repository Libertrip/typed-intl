...
function localizedMessages<M extends Messages>(
  messages: MessagesParameter<M>
): LocalizedMessages<M> {
...
export type MessagesOf<T> = T extends MessageProvider<infer M> ? M : never
 ...
 */
class ExtendingMessageProvider<
  B extends Messages,
  M extends Messages
> extends CachedMessageProvider<B & M> {
  /**
   ...
   */
  constructor(
    private readonly base: MessageProvider<B>,
    private readonly extension: MessageProvider<M>
  ) {
    super()
  }
...
class TranslatorImpl<M extends Messages> extends CachedMessageProvider<M> implements Translator<M> {
  ...
  constructor(
    private readonly defaultMessages: LocalizedMessages<M>,
  ...
  partiallySupporting(
    language: Language,
    translation: MessagesParameter<Partial<M>>
  ): Translator<M> {
  ...
  private buildRecursive(
    ...
    if (translationLanguage) {
      const translation = this.translations.get(translationLanguage)
      if (translation) {
        const mergedBase = this.buildRecursive(
          base,
          requestedLanguage,
          translationLanguage.parent()
        )
...
export function pickPreferredLanguage(
  ...
  const bestLanguagePreference = usersPreferredLanguages.find(navLang =>
    navLang.matchesOneOf(availableTranslations)
  )
  return bestLanguagePreference || usersPreferredLanguages[0]
}

let _preferredLanguage: LanguageTag | undefined =
...
const navigatorLanguages =
  typeof navigator !== 'undefined'
    ? navigator.languages
      ? navigator.languages
      : /* istanbul ignore next: not reachable in test */ navigator.language
      ? [navigator.language]
      : ['en']
    : ['en']
...
export function selectPreferredLanguage(
  availableTranslations: ReadonlyArray<string>,
  usersPreferredLanguages: ReadonlyArray<string> = navigatorLanguages
): void {
